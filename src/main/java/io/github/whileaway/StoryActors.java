...
import io.github.whileaway.core.ActorPresencePolicy;
...
public final class StoryActors {
    ...
    public static StoryActorRecord prepare(Entity actor,String id,String event,String role,UUID instance,UUID owner,boolean temporary) {
        return prepare(actor,id,event,role,instance,owner,temporary,r->{});
    }
    /** Adapter facts must join the initial durable reservation, never a second half-initializing write. */
    public static StoryActorRecord prepare(Entity actor,String id,String event,String role,UUID instance,UUID owner,boolean temporary,
            java.util.function.Consumer<StoryActorRecord> initialize) {
        ...
        initialize.accept(r);d.actors.put(id,r);tag(actor,r);capture(actor,r);persist(level.getServer());emit(level.getServer(),r,"PREPARED");return r;
    }
    ...
    public static boolean ownerPresent(MinecraftServer server,StoryActorRecord r) {
        if(NpcEvents.isNpc(r))return NpcEvents.hasWitness(server,r);
        ...
        return ActorPresencePolicy.canRun(true,p!=null,p!=null&&p.isAlive(),
            p!=null&&p.level().dimension().location().toString().equals(r.dimension),
    ...
    public static List<String> integrity(MinecraftServer server) {
        ...
        for(var r:d.actors.values()) {
            if(NpcEvents.isNpc(r))result.addAll(NpcState.integrity(r));
