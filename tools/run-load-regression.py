...
W=(R/'../../work').resolve()
SAVES=W/'load-guard-exceptions/saves'
...
def run(task,kind):
    ...
    start=time.time()
    script="$ErrorActionPreference='Stop'; ./tools/select-test-monitor.ps1 -OutputPath evidence/load-guard/monitor.json; $env:JAVA_HOME='C:\\Program Files\\Eclipse Adoptium\\jdk-21.0.5.11-hotspot'; $env:GRADLE_USER_HOME=(Resolve-Path ../../work/gradle-home).Path; ./gradlew.bat --offline "+task+"; exit $LASTEXITCODE"
