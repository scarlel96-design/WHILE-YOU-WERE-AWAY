...
MOD = D/'whileaway-0.3.1-load-guard.jar'
sha = lambda p: hashlib.sha256(p.read_bytes()).hexdigest()
baseline_hash = '3d3b08beddd7e9fae87c41188c31251e941ee9da9b6697b50bcf96a2fd960766'
source_hash = '21175125bdf2ac451aaabf8aaf51a2cb533f016a39aaf8df740bebbbf61785aa'
assert sha(BASE) == baseline_hash
assert sha(E/'baseline/previous-source.zip') == source_hash
assert sha(R/'dist/wayfarer-stability/whileaway-0.3.1-wayfarer-stability.jar') == baseline_hash
assert sha(R/'dist/wayfarer-stability/whileaway-0.3.1-wayfarer-stability-source.zip') == source_hash
...
'''.replace('BASE_HASH',baseline_hash).replace('MOD_HASH',sha(MOD))
    ...
    import json
    assert json.loads((E/'GATES.json').read_text())['load_guard']=='PASS'
    ...
    files += [p for folder in ('load-guard',) for p in (R/'evidence'/folder).rglob('*') if p.is_file() and p.suffix not in ('.jar','.zip')]
    ...
    archive = D/'whileaway-0.3.1-load-guard-source.zip'
