...
expected=['Chunk1','Chunk2','Chunk4','Nether1','Nether2','End1','End2','CutNether2','ResumeNether2']
assert [r['mode'] for r in rows]==expected and len({r['pid'] for r in rows})==9
for row in rows:
...
assert 'PASS core assertions=176857' in (E/'tests-4.log').read_text(encoding='utf-8-sig')
g={'version':'0.3.1-dev.1','work':'npc-world','saveFormat':4,'actorSchema':1,'artRevision':2,'world_travel':'PASS','actualChunkUnloadReload':'PASS checkpoints 1,2,4','unloadedNotMissing':'PASS','Nether':'PASS checkpoints 1,2','End':'PASS checkpoints 1,2','dimensionFreshProcess':'PASS Nether checkpoint 2','gameTests':58,'coreAssertions':176857,'realClients':9,'unchangedBaselineClasses':83,'NPC_overall':'PARTIAL','campaign':'PARTIAL','copyDivergence':'NOT TESTED','residencePathLight':'NOT TESTED','NPC_corruptionClient':'NOT TESTED','deathABCDE':'BASELINE PASS; not rerun in this slice','composite':'NOT STARTED'}
(E/'GATES.json').write_text(json.dumps(g,indent=2)+'\n');print('PASS NPC world travel: actual unload/reload 3 checkpoints; Nether/End each 2; Nether cut/fresh process; identity/facts/disk/seed preservation; NPC overall PARTIAL')
