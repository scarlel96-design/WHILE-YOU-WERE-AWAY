...
D = R/'dist/wayfarer-stability'
BASE = E/'baseline/previous.jar'
MOD = D/'whileaway-0.3.1-wayfarer-stability.jar'
sha = lambda p: hashlib.sha256(p.read_bytes()).hexdigest()
baseline_hash = '9c5f78105b92a1afae46b4d488fc67aff2e0288ca78fd90f84aacb9b32efedf8'
source_hash = '81b47fcca580ac17102e0135c963bc1b2fdce638544ee5b82571b4b65fdc55ec'
assert sha(BASE) == baseline_hash
assert sha(E/'baseline/previous-source.zip') == source_hash
assert sha(R/'dist/campaign-pilot/whileaway-0.3.1-campaign-pilot.jar') == baseline_hash
assert sha(R/'dist/campaign-pilot/whileaway-0.3.1-campaign-pilot-source.zip') == source_hash
...
'''.replace('BASE_HASH',baseline_hash).replace('MOD_HASH',sha(MOD))
    ...
    import json
    assert json.loads((E/'GATES.json').read_text())['wayfarer_suite']=='PASS'
    assert 'All 48 required tests passed' in (E/'modified-tests.log').read_text(encoding='utf-8-sig')
    ...
    files += [p for folder in ('wayfarer-stability',) for p in (R/'evidence'/folder).rglob('*') if p.is_file() and p.suffix not in ('.jar','.zip')]
    ...
    archive = D/'whileaway-0.3.1-wayfarer-stability-source.zip'
    with zipfile.ZipFile(archive,'w',zipfile.ZIP_DEFLATED) as z:
        for p in files:
            info=zipfile.ZipInfo('while-you-were-away/'+p.relative_to(R).as_posix(),(2026,9,11,0,0,0))
