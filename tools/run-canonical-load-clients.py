...
R=Path(__file__).resolve().parents[1];E=R/'evidence/canonical-recovery';W=(R/'../../work/canonical-recovery-client-final/saves').resolve()
...
for mode in ('Control','Recover','RecoverReload','Blocked','Unsupported'):
    ...
    script="$ErrorActionPreference='Stop'; ./tools/select-test-monitor.ps1 -OutputPath evidence/canonical-recovery/monitor.json; $env:JAVA_HOME='C:\\Program Files\\Eclipse Adoptium\\jdk-21.0.5.11-hotspot'; $env:GRADLE_USER_HOME=(Resolve-Path ../../work/gradle-home).Path; ./gradlew.bat --offline runLoadGuard"+mode+"; exit $LASTEXITCODE"
    start=time.time()
