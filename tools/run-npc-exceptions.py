...
R=Path(__file__).resolve().parents[1];E=R/'evidence/npc-exceptions';W=(R/'../../work/npc-exceptions/saves').resolve()
...
for mode in modes:
    ...
    script="$ErrorActionPreference='Stop'; ./tools/select-test-monitor.ps1 -OutputPath evidence/npc-exceptions/monitor.json; $env:JAVA_HOME='C:\\Program Files\\Eclipse Adoptium\\jdk-21.0.5.11-hotspot'; $env:GRADLE_USER_HOME=(Resolve-Path ../../work/gradle-home).Path; ./gradlew.bat --offline runNpcException"+mode+"; exit $LASTEXITCODE"
    start=time.time()
    ...
    row={'mode':mode,'command':'./gradlew.bat --offline runNpcException'+mode,'exit':result.returncode,'pid':pid,'processExited':bool(stopped),'seconds':time.time()-start,'pass':good}
