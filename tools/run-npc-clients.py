...
R=Path(__file__).resolve().parents[1];E=R/'evidence/npc-lifecycle';W=(R/'../../work/npc-lifecycle/saves').resolve()
sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
def manifest(p):return {f.relative_to(p).as_posix():sha(f) for f in p.rglob('*') if f.is_file()}
W.mkdir(parents=True,exist_ok=True)
modes=sys.argv[1:] or ['Seed']+[m for c in 'ABCDE' for m in ('Cut'+c,'Resume'+c)]
for mode in modes:
    ...
    script="$ErrorActionPreference='Stop'; ./tools/select-test-monitor.ps1 -OutputPath evidence/npc-lifecycle/monitor.json; $env:JAVA_HOME='C:\\Program Files\\Eclipse Adoptium\\jdk-21.0.5.11-hotspot'; $env:GRADLE_USER_HOME=(Resolve-Path ../../work/gradle-home).Path; ./gradlew.bat --offline runNpc"+mode+"; exit $LASTEXITCODE"
    start=time.time()
    ...
    row={'mode':mode,'command':'./gradlew.bat --offline runNpc'+mode,'exit':result.returncode,'pid':pid,'processExited':bool(stopped),'seconds':time.time()-start,'pass':good}
