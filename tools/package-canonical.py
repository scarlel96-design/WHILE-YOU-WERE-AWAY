...
E = R/'evidence/canonical-recovery'
D = R/'dist/canonical-recovery'
BASE = E/'baseline/previous.jar'
MOD = D/'whileaway-0.3.1-canonical-recovery.jar'
sha = lambda p: hashlib.sha256(p.read_bytes()).hexdigest()
baseline_hash = 'a66cf05706329faab022af12d9ab016dc61b8e719724858f70c3449f5ae8ad03'
source_hash = 'c5d0161e2f436dcd8cb6c10e4654c84656e5a91cf1b96991c19907e8aa1f38b0'
assert sha(BASE) == baseline_hash
assert sha(E/'baseline/previous-source.zip') == source_hash
assert sha(R/'dist/load-guard/whileaway-0.3.1-load-guard.jar') == baseline_hash
assert sha(R/'dist/load-guard/whileaway-0.3.1-load-guard-source.zip') == source_hash
...
'''.replace('BASE_HASH',baseline_hash).replace('MOD_HASH',sha(MOD))
    ...
    import json
    assert json.loads((E/'GATES.json').read_text())['canonical_suite']=='PASS'
    ...
    files += [p for folder in ('canonical-recovery',) for p in (R/'evidence'/folder).rglob('*') if p.is_file() and p.suffix not in ('.jar','.zip')]
    ...
    archive = D/'whileaway-0.3.1-canonical-recovery-source.zip'
