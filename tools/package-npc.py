...
MOD = D/'whileaway-0.3.1-npc-lifecycle.jar'
sha = lambda p: hashlib.sha256(p.read_bytes()).hexdigest()
baseline_hash = '82b0201c0067bc50fe931e0daf24b54c2bef31cd07e2cc63845ae4677beca542'
source_hash = '987953a2ceaa5edd0a97f5c285015d3e5611e685566182618b6910d6857eaaee'
assert sha(BASE) == baseline_hash
assert sha(E/'baseline/previous-source.zip') == source_hash
assert sha(R/'dist/canonical-recovery/whileaway-0.3.1-canonical-recovery.jar') == baseline_hash
assert sha(R/'dist/canonical-recovery/whileaway-0.3.1-canonical-recovery-source.zip') == source_hash
...
if '--prepare' in sys.argv:
    ...
    with zipfile.ZipFile(E/'baseline/previous-source.zip') as old:
        ...
        for name in ('ActorGameTests.java','StoryGameTests.java','CanonicalGameTests.java'):
...
'''.replace('BASE_HASH',baseline_hash).replace('MOD_HASH',sha(MOD))
    ...
    with zipfile.ZipFile(BASE) as old, zipfile.ZipFile(MOD) as new:
        ...
        assert all(old.read(n) == new.read(n) for n in assets if n not in ('assets/whileaway/lang/en_us.json','assets/whileaway/lang/ko_kr.json'))
    ...
    text = f'PASS archive reopened; runtime classes match={len(classes)}; asset/data entries={len(assets)} (only 2 language files changed); baseline GameTest sources unchanged=3\nBASE_SHA256={sha(BASE)}\nMOD_SHA256={sha(MOD)}\n'
    (E/'ARCHIVE_PROOF.txt').write_text(text,encoding='utf-8')
    print(text)
else:
    import json
    assert json.loads((E/'GATES.json').read_text())['npc_abcde']=='PASS'
    ...
    files += [p for folder in ('npc-lifecycle',) for p in (R/'evidence'/folder).rglob('*') if p.is_file() and p.suffix not in ('.jar','.zip')]
    ...
    archive = D/'whileaway-0.3.1-npc-lifecycle-source.zip'
