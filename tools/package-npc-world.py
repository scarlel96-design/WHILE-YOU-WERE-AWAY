...
E = R/'evidence/npc-world'
D = R/'dist/npc-world'
BASE = E/'baseline/previous.jar'
MOD = D/'whileaway-0.3.1-npc-world.jar'
sha = lambda p: hashlib.sha256(p.read_bytes()).hexdigest()
baseline_hash = '19636c1b8d8c8b9faf7693f5c05e69b199bb896c4f131c7f72c34331ca099c19'
source_hash = 'd34f61add10bcfd2ff44fe6cfc4034f74abab503d0f074eb0940b55646afeb4d'
assert sha(BASE) == baseline_hash
assert sha(E/'baseline/previous-source.zip') == source_hash
assert sha(R/'dist/npc-exceptions/whileaway-0.3.1-npc-exceptions.jar') == baseline_hash
assert sha(R/'dist/npc-exceptions/whileaway-0.3.1-npc-exceptions-source.zip') == source_hash
...
if '--prepare' in sys.argv:
    ...
    with zipfile.ZipFile(E/'baseline/previous-source.zip') as old:
        ...
        for name in ('ActorGameTests.java','StoryGameTests.java','CanonicalGameTests.java','NpcGameTests.java','NpcExceptionGameTests.java'):
...
'''.replace('BASE_HASH',baseline_hash).replace('MOD_HASH',sha(MOD))
    ...
    with zipfile.ZipFile(BASE) as old, zipfile.ZipFile(MOD) as new:
        ...
        previous_classes = [n for n in old.namelist() if n.endswith('.class')]
        assert all(old.read(n) == new.read(n) for n in previous_classes), 'Baseline class changed'
    text = f'PASS archive reopened; runtime classes match={len(classes)}; asset/data entries={len(assets)} (all baseline assets/data unchanged); baseline GameTest sources unchanged=5\nBASE_SHA256={sha(BASE)}\nMOD_SHA256={sha(MOD)}\n'
    (E/'ARCHIVE_PROOF.txt').write_text(text,encoding='utf-8')
    print(text)
else:
    import json
    gates=json.loads((E/'GATES.json').read_text())
    assert gates.get('world_travel')=='PASS' or (gates.get('safePause') and gates.get('buildableCheckpoint')=='PASS')
    ...
    files += [p for folder in ('npc-world',) for p in (R/'evidence'/folder).rglob('*') if p.is_file() and p.suffix not in ('.jar','.zip')]
    ...
    archive = D/'whileaway-0.3.1-npc-world-source.zip'
    with zipfile.ZipFile(archive,'w',zipfile.ZIP_DEFLATED) as z:
        for p in files:
            info=zipfile.ZipInfo('while-you-were-away/'+p.relative_to(R).as_posix(),(2026,9,14,0,0,0))
