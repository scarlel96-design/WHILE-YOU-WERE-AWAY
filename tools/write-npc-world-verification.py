...
R=Path(__file__).resolve().parents[1];E=R/'evidence/npc-world';D=R/'dist/npc-world'
h=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
g=json.loads((E/'GATES.json').read_text());assert g['world_travel']=='PASS' and g['copyDivergence'].startswith('PASS')
travel=[json.loads(x) for x in (E/'client-commands.jsonl').read_text().splitlines()]
copies=[json.loads(x) for x in (E/'copy-commands.jsonl').read_text().splitlines()]
assert len(travel)==9 and len(copies)==5 and all(r['pass'] for r in travel+copies)
assert 'All 58 required tests passed' in (E/'tests-4.log').read_text(encoding='utf-8-sig')
base=E/'baseline/previous.jar';mod=D/'whileaway-0.3.1-npc-world.jar'
with zipfile.ZipFile(base) as a,zipfile.ZipFile(mod) as b:
    classes=[n for n in a.namelist() if n.endswith('.class')]
    assert len(classes)==83 and all(a.read(n)==b.read(n) for n in classes)
    assert len([n for n in b.namelist() if n.endswith('.class')])==85
text=f'''Current: While You Were Away / 0.3.1-dev.1 / npc-world / PARTIAL
...
BASELINE input JAR: {base}
SHA256 {h(base)}
BASELINE source: {E/'baseline/previous-source.zip'}
SHA256 {h(E/'baseline/previous-source.zip')}
Immediate continuation snapshot (previous npc-world JAR/source) preserved at {E/'history/pause-20260914'}:
...
Travel process durations total={sum(r['seconds'] for r in travel):.3f}s; copy total={sum(r['seconds'] for r in copies):.3f}s. These are launch-to-exit durations, not human playtime.
...
MODIFIED_FILE={mod}
MODIFIED_SHA256={h(mod)}
DIFF_FILE={E/'DIFF.patch'}
VERIFICATION={E/'VERIFICATION.txt'}
ROLLBACK={E/'ROLLBACK.sh'}
SOURCE_ZIP={D/'whileaway-0.3.1-npc-world-source.zip'}

BASELINE / MODIFIED / ROLLBACK exact command/input/output/exit/hash:
{(E/'transaction.json').read_text(encoding='utf-8-sig')}

REAL CLIENT command ledger (exit codes / PIDs / durations):
{json.dumps(travel+copies,indent=2)}
'''
(E/'VERIFICATION.txt').write_text(text,encoding='utf-8')
(R/'docs/NPC_WORLD_031.md').write_text(text.split('GATES\n')[0],encoding='utf-8')
print('PASS detailed current verification written; no full NPC/campaign promotion')
