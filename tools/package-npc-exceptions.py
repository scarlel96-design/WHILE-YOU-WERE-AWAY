...
E = R/'evidence/npc-exceptions'
D = R/'dist/npc-exceptions'
BASE = E/'baseline/previous.jar'
MOD = D/'whileaway-0.3.1-npc-exceptions.jar'
sha = lambda p: hashlib.sha256(p.read_bytes()).hexdigest()
baseline_hash = '80a80cf1dd741744c4b6c7c2bed21264437d651c5421d9198f13b40e3ca79e51'
source_hash = 'eba768dec2a1784f71fe371573a92daa9f1cc92b8f3a5cb40463cf9d958ff736'
assert sha(BASE) == baseline_hash
assert sha(E/'baseline/previous-source.zip') == source_hash
assert sha(R/'dist/npc-lifecycle/whileaway-0.3.1-npc-lifecycle.jar') == baseline_hash
assert sha(R/'dist/npc-lifecycle/whileaway-0.3.1-npc-lifecycle-source.zip') == source_hash
...
if '--prepare' in sys.argv:
    ...
    with zipfile.ZipFile(E/'baseline/previous-source.zip') as old:
        ...
        for name in ('ActorGameTests.java','StoryGameTests.java','CanonicalGameTests.java','NpcGameTests.java'):
...
'''.replace('BASE_HASH',baseline_hash).replace('MOD_HASH',sha(MOD))
    ...
    text = f'PASS archive reopened; runtime classes match={len(classes)}; asset/data entries={len(assets)} (all baseline assets/data unchanged); baseline GameTest sources unchanged=4\nBASE_SHA256={sha(BASE)}\nMOD_SHA256={sha(MOD)}\n'
    (E/'ARCHIVE_PROOF.txt').write_text(text,encoding='utf-8')
    print(text)
else:
    import json
    gates=json.loads((E/'GATES.json').read_text())
    assert gates.get('death_bundle')=='PASS' or (gates.get('safePause') and gates.get('buildableCheckpoint')=='PASS')
    ...
    files += [p for folder in ('npc-exceptions',) for p in (R/'evidence'/folder).rglob('*') if p.is_file() and p.suffix not in ('.jar','.zip')]
    ...
    archive = D/'whileaway-0.3.1-npc-exceptions-source.zip'
